...
test "output: trim float64 and raw from start" {
    ...
    try testing.expectEqual(@as(f64, 1000.0), out.float64(0, 0).?);
    try testing.expectEqual(@as(f64, 2000.0), out.float64(0, 1).?);

    const r0 = out.raw(1, 0).?;
    try testing.expectEqualStrings("ab", r0.ptr);
    const r1 = out.raw(1, 1).?;
...
test "output: trim float64 and raw with offset" {
    ...
    try testing.expectEqual(@as(usize, 2), out.num_rows);
    try testing.expectEqual(@as(f64, 2000.0), out.float64(0, 0).?);
    try testing.expectEqual(@as(f64, 3000.0), out.float64(0, 1).?);
...
test "output: accessors" {
    ...
    try testing.expectEqual(@as(usize, 5), out.num_rows);
    try testing.expectEqual(OutputType.Float64, out.dimensionType(0).?);
    try testing.expectEqual(OutputType.Raw, out.dimensionType(1).?);

    // Float64 data accessible
    try testing.expect(out.float64(0, 0) != null);
    // Raw data accessible
    try testing.expect(out.raw(1, 0) != null);

    // Out-of-bounds dimension returns null/None
    try testing.expect(out.dimensionType(2) == null);
    try testing.expect(out.float64(2, 0) == null);
    try testing.expect(out.raw(2, 0) == null);
}
